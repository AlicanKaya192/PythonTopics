...
# 1. Exploratory Data Analysis
################################################
# Veri setine genel bir bakış atılır.
# Değişkenlerin tipleri, dağılımları ve eksik değerler incelenir.
# Hedef değişkenin (Outcome) dağılımı kontrol edilir.

################################################
# 2. Data Preprocessing & Feature Engineering
################################################
# Veri seti modele girmeden önce gerekli ön işleme adımları uygulanır.
# Aykırı değerler, eksik değerler ve değişken dönüşümleri (Encoding, Scaling) yapılır.
# CART (Ağaç yöntemleri) genellikle ölçeklendirmeye (Scaling) ihtiyaç duymaz.
...
#####################
# Veri setini eğitim ve test olarak ayırıp model başarısını test ediyoruz.
...
#####################
# Cross Validation ile modelin genelleme yeteneğini daha doğru ölçüyoruz.
...
# 4. Hyperparameter Optimization with GridSearchCV
################################################
# Modelin hiperparametrelerini optimize ederek başarıyı artırmayı hedefleriz.
# GridSearchCV ile belirlediğimiz parametre setlerinin kombinasyonları denenir.

# Mevcut modelin parametrelerini görme
cart_model.get_params()

# Aranacak parametre setinin belirlenmesi
# max_depth: Ağacın maksimum derinliği.
# min_samples_split: Bir düğümün bölünmesi için gereken minimum örnek sayısı.
cart_params = {'max_depth': range(1, 11),
               "min_samples_split": range(2, 20)}

# GridSearchCV nesnesinin oluşturulması ve modelin eğitilmesi
# cv=5: 5 katlı çapraz doğrulama.
# n_jobs=-1: İşlemcilerin tamamını kullan.
# verbose=1: İşlem sırasında raporlama yap.
...
# En iyi skorun (accuracy) bulunması
cart_best_grid.best_score_

# Rastgele bir örneklem ile tahmin yapma
...
# 5. Final Model
################################################
# GridSearchCV ile bulunan en iyi parametreler kullanılarak final model oluşturulur.

# En iyi parametreler ile yeni bir model nesnesi oluşturma ve eğitme
cart_final = DecisionTreeClassifier(**cart_best_grid.best_params_, random_state=17).fit(X, y)
cart_final.get_params()

# Alternatif olarak mevcut modele en iyi parametreleri atama
cart_final = cart_model.set_params(**cart_best_grid.best_params_).fit(X, y)

# Final modelin performansını CV ile değerlendirme
...
# 6. Feature Importance
################################################
# Modelin hangi değişkenlere ne kadar önem verdiğini inceleriz.
# Bu, modelin kararlarını nasıl verdiğini anlamamıza yardımcı olur.
...
# 7. Analyzing Model Complexity with Learning Curves (BONUS)
################################################
# Model karmaşıklığının (örneğin max_depth) model başarısı üzerindeki etkisini görselleştiririz.
...
# Bu işlemi fonksiyonlaştırarak daha genel hale getirebiliriz.
...
# 8. Visualizing the Decision Tree
################################################
# Oluşturulan karar ağacının görselleştirilmesi.
# Bu işlem için graphviz kütüphanesinin sistemde kurulu olması gerekir.
...
# 9. Extracting Decision Rules
################################################
# Karar ağacının kurallarını metin formatında çıkarır.
# Hangi değişkenin hangi değerine göre dallanma yapıldığını gösterir.
...
################################################
# Karar kurallarını Python, SQL veya Excel formatında koda dönüştürür.
# Bu sayede model canlı sisteme entegre edilebilir.
...
# 11. Prediction using Python Codes
################################################
# Skompile ile üretilen Python kodunu kullanarak tahmin yapma fonksiyonu.
# Bu fonksiyon, scikit-learn bağımlılığı olmadan tahmin yapmayı sağlar.
...
# 12. Saving and Loading Model
################################################
# Eğitilen modeli diske kaydetme ve tekrar yükleme işlemleri.
