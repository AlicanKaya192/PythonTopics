...
#from FLOMusteriSegmentasyonu.FLO_RFM import analysis_date
...
# 2. Aykırı değerleri baskılamak için gerekli olan outlier_thresholds ve replace_with_thresholds fonksiyonlarını tanımlayınız.
# Not: cltv hesaplanırken frequency değerleri integer olması gerekmektedir.Bu nedenle alt ve üst limitlerini round() ile yuvarlayınız.
def outlier_thresholds(dataframe, value):
...
def replace_with_thresholds(dataframe, value):
    low_limit, up_limit = outlier_thresholds(dataframe, value)
    dataframe.loc[(dataframe[value] < low_limit), value] = round(low_limit)
    dataframe.loc[(dataframe[value] > up_limit), value] = round(up_limit)
...
cltv_df["cltv_segment"] = pd.qcut(cltv_df["cltv"], 2, labels=["B", "A"])
...
cltv_df[["cltv_segment", "recency_cltv_weekly", "frequency", "monetary_cltv_avg"]].groupby("cltv_segment").agg(["mean", "std"])
